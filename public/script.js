...
function createMediaDB(type) {
    ...
    // NEW: threshold for switching to direct upload (under Vercel 4.5 MB body limit)
    ...
    function _uploadSingleFile(formData, category, onProgress) {
        return new Promise((resolve, reject) => {
            ...
            const queryUrl = `${_apiBase}/uploads?category=${encodeURIComponent(category)}&lastModified=${timestamp}`;
    ...
    // ============================================================
    // NEW: Direct-upload helpers (bypass Vercel 4.5 MB body limit)
    // ============================================================

    // Step 1: ask the server for a presigned PUT URL
    function _requestPresign(file, category) {
        const url = _apiBase + '/uploads?action=presign&date=' + Date.now();
    ...
    // Step 2: PUT the raw file directly to blob storage
    function _putToPresignedUrl(file, presignedUrl, onProgress) {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            xhr.open('PUT', presignedUrl, true);
            // No Authorization header — URL is self-signed.
            xhr.upload.onprogress = e => {
                if (e.lengthComputable && typeof onProgress === 'function') {
                    onProgress(Math.round((e.loaded / e.total) * 100));
                }
    ...
    // Dispatcher: small files → existing path, large files → direct PUT
    ...
    // CHANGED: now routes through _uploadFileSmart (small → API, large → direct PUT)
    function _uploadImage(file, onProgress) {
        return _uploadFileSmart(file, onProgress, _uploadCategory);
    }
    ...
    function _uploadVideo(file, onProgress) {
        return new Promise((resolve, reject) => {
            _generateThumbnail(file, thumbnailFile => {
                ...
                // CHANGED: video bytes now go through _uploadFileSmart.
                //          Thumbnail below stays on _uploadSingleFile (always small).
                _uploadFileSmart(
                    file,
                    p => onProgress && onProgress({ phase: 'video', percent: p * 0.7, fileName: file.name }),
                    videoCategory
                )
