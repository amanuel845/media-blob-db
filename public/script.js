...
function createMediaDB(type) {
    ...
    // Threshold for switching to direct upload (under Vercel 4.5 MB body limit)
    ...
    function _uploadSingleFile(formData, category, onProgress) {
        return new Promise((resolve, reject) => {
            ...
            const queryUrl = `${_apiBase}/upload?category=${encodeURIComponent(category)}&lastModified=${timestamp}`;
    ...
    function _requestPresign(file, category) {
        const url = _apiBase + '/upload?action=presign&date=' + Date.now();
    ...
    // CHANGED: wrapped so the callback always receives { phase, percent }
    ...
    // CHANGED: sequential — video bytes first (0→90%), then thumbnail (90→100%)
    ...
    function uploadFiles(files, onProgress, categories) {
        ...
        const processNext = index => {
            ...
            return uploadFn(file, p => {
                // CHANGED: clamp to [0,100] and round to integer
...
function createMediaApp(mediaDB, type) {
    ...
    function init() {
        ...
        // CHANGED: display progress via toFixed(0) for clean integer output
